...
(defun user-config-js ()
  ...
  (defun hero/js-read-locale-string (filePath)
    "Return a list of lines of a file at filePath."
    (with-temp-buffer
      (insert-file-contents filePath)
      (seq-filter
       (lambda (x) (not (or (string-prefix-p "export default" x)
                            (string-prefix-p "lang: " x)
                            (string-prefix-p "/*" x)
                            (string-prefix-p "//" x)
                            (string-prefix-p "messages: {" x))))
       (mapcar 's-trim (split-string (buffer-string) "\n" t)))))

  (defun hero/js-insert-i18n ()
    "Insert I18n"
    (interactive)
    (ivy-read "EH I18n "
              (let* ((pr (project-current t))
                     (dirs (car (project-roots pr)))
                     (locale-file (concat dirs "src/packages/eh-locale/lang/en-AU.js")))
                (hero/js-read-locale-string locale-file))
              :action (lambda (app)
                        (let* ((locale-id (car (s-split ":" (s-trim app))))
                               (locale-id-with-quote (if (string-prefix-p "'" locale-id)
                                                         locale-id
                                                       (format "'%s'" locale-id))))
                          (insert (format "Intl.formatMessage({ id: %s })" locale-id-with-quote))))))
  ...
  (evil-leader/set-key-for-mode 'js2-mode "l" 'hero/js-insert-i18n)
  (evil-leader/set-key-for-mode 'rjsx-mode "l" 'hero/js-insert-i18n)
...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (overseer neotree nameless macrostep helm-xref helm-themes helm-swoop helm-purpose helm-projectile helm-mode-manager helm-flx helm-descbinds helm-ag evil-mc elisp-slime-nav define-word auto-compile packed ace-jump-helm-line yasnippet-snippets yaml-mode xterm-color ws-butler writeroom-mode winum which-key wgrep web-mode web-beautify volatile-highlights vimrc-mode vi-tilde-fringe uuidgen use-package unfill treemacs-projectile treemacs-evil toc-org tagedit symon string-inflection sql-indent spaceline-all-the-icons smex smeargle slim-mode shell-pop seeing-is-believing scss-mode sass-mode rvm ruby-tools ruby-test-mode ruby-refactor ruby-hash-syntax rubocop rspec-mode robe rjsx-mode reveal-in-osx-finder restart-emacs request read-aloud rbenv rainbow-delimiters pug-mode projectile-rails prodigy prettier-js popwin persp-mode password-generator paradox osx-trash osx-dictionary osx-clipboard orgit org-projectile org-present org-pomodoro org-mime org-download org-bullets org-brain open-junk-file ob-coffeescript nord-theme mwim multi-term move-text mmm-mode minitest markdown-toc magithub magit-svn magit-gitflow magit-gh-pulls madhat2r-theme lorem-ipsum livid-mode link-hint launchctl json-navigator js2-refactor js-doc ivy-yasnippet ivy-xref ivy-purpose ivy-hydra indent-guide impatient-mode idea-darkula-theme hungry-delete hl-todo highlight-parentheses highlight-numbers highlight-indentation helm-make google-translate golden-ratio gnuplot gitignore-templates gitignore-mode github-search github-clone gitconfig-mode gitattributes-mode git-timemachine git-messenger git-link git-gutter-fringe git-gutter-fringe+ gist gh-md fuzzy font-lock+ flycheck-pos-tip flx-ido fill-column-indicator feature-mode fancy-battery eyebrowse expand-region evil-visualstar evil-visual-mark-mode evil-unimpaired evil-tutor evil-terminal-cursor-changer evil-surround evil-org evil-numbers evil-nerd-commenter evil-matchit evil-magit evil-lisp-state evil-lion evil-indent-plus evil-iedit-state evil-goggles evil-exchange evil-escape evil-ediff evil-cleverparens evil-args evil-anzu eval-sexp-fu eshell-z eshell-prompt-extras esh-help engine-mode emojify emoji-cheat-sheet-plus emmet-mode editorconfig dumb-jump dotenv-mode doom-themes doom-modeline dockerfile-mode docker diminish diff-hl dactyl-mode csv-mode counsel-projectile counsel-css copy-as-format company-web company-tern company-statistics company-emoji column-enforce-mode coffee-mode clean-aindent-mode chruby centered-cursor-mode bundler browse-at-remote base16-theme auto-yasnippet auto-highlight-symbol atom-dark-theme apib-mode aggressive-indent add-node-modules-path ace-link ac-ispell))))
(custom-set-faces
